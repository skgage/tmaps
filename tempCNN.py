...
with tf.Graph().as_default() as g:
	...
	subtract = 0.5*squaring - dr_log_nonan
...
with tf.Session(graph=g) as sess:
    ...
    for i in range(20):
		...
		res = sess.run(output, {is_training: False})
	    ...
	    #train_data = sess.run(x)
		r0 = res[:,0]
		r1 = res[:,1]
		loss_vec.append(error)
plt.plot(np.linspace(0,i, i+1), np.array(loss_vec))
plt.xlabel('epoch')
...
def kl_divergence(p, q): #tf.reduce_sum(p * tf.log(p/q))
	#with tf.GradientTape() as g:
  		#g.watch(q)
		#dr = g.gradient(p, q)
	dr = tf.gradients(p, q) #p/q
	dr_log = tf.log(dr)
	dr_log = tf.where(tf.is_nan(dr_log), tf.zeros_like(dr_log), dr_log)
	return tf.reduce_sum(p*dr_log)
...
with g.as_default():
    ...
    tcn = TemporalConvNet([4,4,1], 2, 0.25) #num_channels, kernel size, dropout
    ...
    dr_log = tf.log(dr)
    dr_log_nonan = tf.where(tf.is_nan(dr_log), tf.zeros_like(dr_log), dr_log)
    squaring = tf.square(output)
    subtract = 0.5*squaring - dr_log_nonan
    loss = tf.reduce_sum((subtract))/numSamps#/float(numSamps)
    #loss = kl_divergence(output,Xinput)
...
with tf.Session(graph=g) as sess:
	...
	#train_data = np.reshape(genData.Banana(10) ,[5,2,1])
	test_data = np.random.randn(numSamps/5, 2, 1)
	#test_data = np.reshape(genData.Banana(10), [5,2,1])
	#print ('data: ', data)
	...
	for i in range(n):
		...
		#print(res)   
		#print(res[0, :, 0])
		dr_check, drlog_check, drlog_nan_check, subtract_check = sess.run([dr, dr_log, dr_log_nonan, subtract], {Xinput: train_data, is_training: False})
    	squaring_check = sess.run(squaring, {Xinput: train_data, is_training: False})
    	print ('check squaring: ', squaring_check)
    	print ('dr_check: ', dr_check)
    	print ('dr log check: ', drlog_check)
    	print ('dr log remove nan check: ', drlog_nan_check)
    	print ('subtraction check: ', subtract_check)
    	#print ('reduce sum check: ', sum_check/numSamps)
    	print ('Epoch: ',i, ' loss: ', training_error)

...
class GeneralDimension:
    ...
    def Evaluate(self, x):
        ...
        if self.d != 0:
            ...
            #print ('x_nonmono: {}, self.d: {}'.format(x_nonmono,self.d))
            ...
            #print ('self.ws[{}]: {}, self.bs[{}]: {}'.format(0, self.ws[0], 0, self.bs[0]))
            for i in range(1,numLayers):
                xLayers[i] = tf.matmul(xLayers[i-1], self.ws[i]) + self.bs[i]
                if i != numLayers - 1:
                    xLayers[i] = tf.nn.relu(xLayers[i])
                #print ('self.ws[{}]: {}, self.bs[{}]: {}'.format(i, self.ws[i], i, self.bs[i]))
            #print ('self.bs: ', self.bs)
            #xLayers[-1] to be concatenated with the the last/monotone component
            #print ('xLayer[-1]: {} x_latf.squarestcomp: {}'.format(xLayers[-1].shape, x_lastcomp.shape))
            x_mono = tf.reshape(tf.stack([xLayers[-1], x_lastcomp]), [-1, self.d+1]) #input to monolayer    
            #print ('updated x_mono: ', x_mono)
        #print ('wlast: ', self.wlast)
        #print ('')
        monoVar = tf.reshape((self.wlast[-1,:]), [1, self.layerSizes[-1]])
        #print ('other weights: {}, mono part: {}'.format(self.wlast[:-1,:], monoVar))
        if self.d != 0:
        ...
        wmono = tf.reshape(wmono, [self.d+1, self.layerSizes[-1]])
        #print ('weight matrix: ', wmono)
        #print ('weight matrix: ', w)     
        #print ('bias matrix: ', self.bs)     
        #print ('x_mono: {}, wmono: {}'.format(x_mono.shape, wmono.shape))                              
        xlayer = tf.matmul(x_mono, wmono) + self.blast
        #print ('xlayer: ', xlayer)
        groupsIdx = [None]*(len(self.groupSizes) + 1)
        groupsIdx[0] = 0
        for i in range(1,len(self.groupSizes)+1):
            groupsIdx[i] = groupsIdx[i-1]+self.groupSizes[i-1]
        #print ('group indices: ', groupsIdx)

        #groupsMin = [] #g1, g2, and so on
        groupsMin = [None]*len(self.groupSizes)
        for i in range(len(self.groupSizes)):
            #print ('xlayer: {}, groupIdx[i]: {}, groupIdx[i+1]: {}'.format(xlayer.shape, groupsIdx[i], groupsIdx[i+1]))
            group = tf.slice(xlayer, [0,groupsIdx[i]], [-1, groupsIdx[i+1]-groupsIdx[i]]) #[xlayer[:,groupsIdx[i]:groupsIdx[i+1]]
            #print ('group: ', i, ' ', group)
            gMin = tf.reduce_min(group, axis = 1)
            #print ('gMin: ', gMin)
        ...
        Ox = tf.reduce_max(groupsMin, axis = 1)
        #print ('Ox: ', Ox)
...
class MonotoneLayer(tf.keras.layers.Layer):
    def __init__(self, dim):
        ...
        assert (tf.reduce_sum(groupSizes).numpy() == layerSizes[-1]),"Group sizes must sum to layer size."
...
class GaussianKL:
    ...
    def __call__(self):
        ...
        with tf.GradientTape() as g:
            ...
            plt.xlabel('x')
            plt.ylabel('r')
            plt.title('Comparing r vs x')
            plt.pause(0.05)'''
            #print ('r: ', r, r.shape)
            #print ('x: ', x, x.shape)
            grad = g.gradient(r, x)
            #print ('dr: ', grad)
        dr = tf.slice(grad, [0,self.d],[numSamps,1])+0.000000000000001
        print ('dr: ', dr, tf.reduce_sum(dr))
        dr_log = tf.log(dr)
        #print ('dr_log: ', dr_log)
        dr_log_nonan = tf.where(tf.is_nan(dr_log), tf.zeros_like(dr_log), dr_log)
        dr_log_nonan = tf.where(tf.is_inf(tf.math.abs(dr_log_nonan)), tf.zeros_like(dr_log_nonan), dr_log_nonan)
        #print ('dr_log_nonan: ', dr_log_nonan)
        obj = tf.reduce_sum((0.5*tf.square(r) - dr_log_nonan))/float(numSamps)
        #print ('obj: ', obj)
        return obj
